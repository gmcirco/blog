...
On the complaint page, these are linked to an agency that handles the complaint. So we have a small labeled set of complaints with partnered agencies

### Defining our problem

For my purposes I wanted to see how quickly I could spin up a small Qwen3.5 model and have it classify unstructured text. I opted for [Qwen3.5-9B](https://huggingface.co/Qwen/Qwen3.5-9B#video-input) which has about a 20 gig footprint total. I installed it via Ollama and was able to get it running in my terminal in under 10 minutes flat. 

After that I wrote up some pretty generic Python code to wrap the invocation in code to parse the complaints from a dataframe. On top of that I defined some Pydantic schemas to constrain the model and perform type checking on the back-end. One cool thing is that even small models like Qwen can support JSON schemas via `response_format={"type": "json_object"}`. It seems like just a year ago this was a headache with some of the earlier OpenAI models and was not available for any notable small models. 
...
In general I think this bodes well for small LLMs or even "micro" LLMs that can run locally on devices and perform highly specific tasks. For some agencies, being able to deploy a free model locally saves inference costs and also makes business-user agreements with partner companies unnecessary. This is also a big win for agencies that are more privacy focused. You can easily deploy these models internally, and keep the data locked down to the server. Personally, I could see deploying several small models like these in mid-sized departments to help work on repetitive tasks that don't require big LLMs or a lot of overhead. In a world with a lot of big LLMs, it might pay to think *small*!